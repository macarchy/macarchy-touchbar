...
- **The auto-popping volume/brightness HUD scene was not built.** `/proc/bus/input/devices` on this machine lists only `Apple MTP keyboard`: Mac14,7 has no physical volume or brightness keys (the F-row *is* the Touch Bar), so the spec's trigger "while those keys are pressed" has no source. `Bar.current_layout()` returns `scenes.top().layout`, so a priority-20 scene replaces the whole bar and covers the very ± button that fired it; and `omarchy-audio-output-volume` already ends in `omarchy-osd -i volume-high -p N`, so it would be a second copy of a display that exists. The trigger to add it is an external keyboard with a real media row, and it is then a `pactl subscribe` doorbell plus `api.show_scene`, with this module's volume `Slider` as the scene body. `[settings] hud` stays inert, exactly as it is today.
