...
# macarchy-dfr — Lot 1 (socle) — plan d'implémentation

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Remplacer tiny-dfr + omarchy-dfr par un daemon Python qui tient l'écran DRM et le tactile de la Touch Bar, dessine au look macOS, et charge des modules par le registre de plugins Omarchy — au terme du lot, la barre fait au moins ce qu'elle fait aujourd'hui, avec sliders et groupes qui restent ouverts.

**Architecture:** Un process en session utilisateur (`macarchy_dfr/` package, entrée `bin/macarchy-dfr`). Le moteur (DRM par ctypes libdrm, evdev brut, uinput, boucle `selectors`) ne connaît que des `Widget` posés dans un `Layout` ; un `Bar` empile couche Fn > scènes > groupe ouvert > layout courant. Les fonctionnalités sont des modules (`modules/<id>/touchbar.py`, même contrat que les plugins externes) qui ne voient qu'un objet `Api`.
...
**Spec:** `docs/superpowers/specs/2026-09-02-macarchy-dfr-design.md` — lire d'abord, le plan argumente depuis la spec.
...
- Le dépôt est `~/Work/macarchy-dfr` (branche `main`). Le daemon existant (`omarchy-dfr`, tiny-dfr) reste en service jusqu'à la tâche 21.

## Structure des fichiers

```
bin/macarchy-dfr                 CLI : daemon | status | reload | group | screenshot | touch | brightness | <module> <verb…>
macarchy_dfr/
  __init__.py
...
systemd/macarchy-dfr.service
udev/70-macarchy-dfr.rules
install.sh
...
**Files:**
- Create: `macarchy_dfr/__init__.py`, `macarchy_dfr/log.py`, `bin/macarchy-dfr`, `tests/test_smoke.py`, `.gitignore`, `README.md`, `pytest.ini`

**Interfaces:**
- Produces: `macarchy_dfr.log.log(*args)` ; `bin/macarchy-dfr` qui insère la racine du dépôt (résolue à travers le symlink) dans `sys.path` et appelle `macarchy_dfr.daemon.main(sys.argv[1:])`.
...
def test_package_imports():
    assert importlib.import_module("macarchy_dfr")
```
...
`bin/macarchy-dfr` (chmod +x) :
```python
#!/usr/bin/env python3
"""macarchy-dfr - the Touch Bar daemon and its CLI. See docs/superpowers/specs."""
import os
import sys
...
from macarchy_dfr.daemon import main  # noqa: E402

sys.exit(main(sys.argv[1:]) or 0)
```

`macarchy_dfr/daemon.py` (provisoire, complété tâche 20) :
```python
def main(argv):
    print("macarchy-dfr: not wired yet", argv)
    return 0
```
...
**Files:**
- Create: `macarchy_dfr/geometry.py`, `macarchy_dfr/layout.py`, `tests/test_layout.py`
...
**Files:**
- Create: `macarchy_dfr/touch.py`, `tests/test_touch.py`
...
**Files:**
- Create: `macarchy_dfr/loop.py`, `tests/test_loop.py`
...
**Files:**
- Create: `macarchy_dfr/output.py`, `tests/test_output.py`
...
**Files:**
- Create: `macarchy_dfr/draw.py`, `tests/test_draw.py`, `fonts/README.md`
...
**Files:**
- Create: `macarchy_dfr/widgets.py`, `tests/test_widgets.py`
...
**Files:**
- Modify: `macarchy_dfr/widgets.py`
...
```python
from macarchy_dfr.widgets import Slider, Meter, Sprite
...
**Files:**
- Create: `macarchy_dfr/scenes.py`, `macarchy_dfr/groups.py`, `tests/test_scenes.py`
...
**Files:**
- Create: `macarchy_dfr/config.py`, `tests/test_config.py`, `config/layouts.toml`
...
`config/layouts.toml` :
```toml
# macarchy-dfr — Touch Bar layouts. Saved changes apply immediately.
...
**Files:**
- Create: `macarchy_dfr/modules.py`, `tests/test_modules.py`, `modules/core/manifest.json` (minimal, pour le test de découverte)
...
- Produces :
  ...
  - `ModuleHost(loop, bar_hooks, registry)` : `.load(spec)` importe le fichier (`importlib.util.spec_from_file_location`, nom `macarchy_dfr_module_<id>`), instancie `Module()`, appelle `setup(api)` ; `.unload(id)` (teardown, annule minuteurs et fds, retire les fabriques) ; `.reload(spec)` ; `.broken: dict[id, str]` ; `.guard(module_id, fn, *a)` exécute sous try/except → en panne + `log`, renvoie `None` en cas d'échec ; `.dispatch_ipc(module_id, verb, args) -> str`.
...
**Files:**
- Create: `macarchy_dfr/bar.py`, `tests/test_bar.py`
...
**Files:**
- Create: `macarchy_dfr/ipc.py`, `tests/test_ipc.py`

**Interfaces:**
- Produces :
  - `sock_path() -> str` = `$XDG_RUNTIME_DIR/macarchy-dfr/sock`.
...
**Files:**
- Create: `macarchy_dfr/hypr.py`, `macarchy_dfr/fnkey.py`, `tests/test_hypr.py`
- Reference: `~/.local/bin/omarchy-dfr` lignes 55–90 (`hypr_dir`, `hypr_query`) et 554–586 (`current_state`) — à porter, pas à réinventer.
...
**Files:**
- Create: `macarchy_dfr/uinput.py`, `tests/test_uinput.py`, `udev/70-macarchy-dfr.rules`
...
- Produces :
  ...
  - `VirtualKeyboard(path="/dev/uinput")` : crée le device « macarchy-dfr keyboard » avec **toutes** les touches 1..0x2ff ; `.press(names)` = down dans l'ordre, SYN, up à l'envers, SYN ; `.close()`. Ne lève pas si `/dev/uinput` est inaccessible : `.available == False`, `press` journalise et ne fait rien.
...
`udev/70-macarchy-dfr.rules` :
```
# macarchy-dfr runs in the user session: it needs the Touch Bar's touch device
...
**Files:**
- Create: `macarchy_dfr/backlight.py`, `tests/test_backlight.py`
...
- Produces :
  ...
  - `display.auto` : Button `brightness_auto`, `active` = `omarchy-als.pid` présent dans `$XDG_RUNTIME_DIR` **et** `omarchy-als.paused` absent (poll 5 s) ; tap → `omarchy-als toggle` puis re-poll après 0.5 s.
...
- Produces :
  ...
  - `system.charge_limit` : Button `battery_saver`, `active` = `charge_control_end_threshold` ≤ 80 (poll 10 s), tap → `omarchy-battery-limit toggle` + re-poll 1 s.
...
from macarchy_dfr.layout import Layout, Row
from macarchy_dfr.widgets import Button, Label, Spacer
...
- Reference: `~/.local/bin/omarchy-dfr` lignes 463–525 (`NotifyParser`, `start_notify_monitor`) — porter tel quel.
...
**Files:**
- Modify: `macarchy_dfr/daemon.py`
- Create: `systemd/macarchy-dfr.service`, `install.sh`, `tests/test_daemon.py`

**Interfaces:**
- Produces `daemon.main(argv)` :
  - `daemon [--headless] [--config PATH]` : construit `EventLoop`, `Output` (`DrmOutput.open()` ou `HeadlessOutput`), `Painter`, `Registry`, `ModuleHost`, charge les modules (`discover(ROOT/modules, ~/.config/omarchy/plugins, shell.json)`), `Config.load(~/.config/macarchy-dfr/layouts.toml)` (repli sur `ROOT/config/layouts.toml`), `Bar`, `VirtualKeyboard` (posé sur `bar.keyboard`), `BarBacklight` (posé sur `bar.backlight`), `TouchReader.open(find_touch_device(), flip=settings.touch_flip)` + `GestureRecognizer` → `bar.gesture` (avec la règle « barre éteinte : le premier `press` réveille et avale le geste jusqu'au `release` »), `FnWatcher` → `bar.set_fn`, `HyprEvents` → `bar.set_context(current_context(prev))`, `IpcServer` avec `EngineIpc`, inotify-par-poll sur layouts.toml et sur chaque `spec.path` de module (`api.watch_file` n'existe pas ici : `loop.every(1, …)` sur les mtimes) → `reload`, `loop.every(0.1, bar.tick)`, `loop.every(1/8, tick des Sprites)`, SIGTERM/SIGINT → `loop.stop()` puis `output.close()`, `keyboard.close()`. Écrit `$XDG_RUNTIME_DIR/macarchy-dfr/daemon.pid`.
...
```ini
[Unit]
Description=macarchy-dfr - the Touch Bar, drawn by us
After=graphical-session.target
PartOf=graphical-session.target

[Service]
Type=simple
ExecStart=%h/.local/bin/macarchy-dfr daemon
Restart=on-failure
RestartSec=2
...
- [ ] **Step 4 : vert (test headless).** Puis `python3 bin/macarchy-dfr daemon --headless &` ; `macarchy-dfr status` ; `macarchy-dfr screenshot /tmp/claude-1001/…/scratchpad/bar.png` ; lire le PNG ; `macarchy-dfr touch 1900,30` (le groupe système s'ouvre) ; nouveau screenshot ; `kill %1`.

- [ ] **Step 5 : première mise sous tension.** `./install.sh`. Si le groupe `video` vient d'être ajouté, une déconnexion est nécessaire ; en attendant, tester en une seule invite : `pkexec bash -c "systemctl stop tiny-dfr; sudo -u $USER XDG_RUNTIME_DIR=/run/user/$(id -u) HYPRLAND_INSTANCE_SIGNATURE=$HYPRLAND_INSTANCE_SIGNATURE setpriv --reuid=$(id -u) --regid=$(id -g) --groups=video,input $PWD/bin/macarchy-dfr daemon"` n'est pas praticable proprement — préférer la déconnexion. Après relogin : `systemctl --user status macarchy-dfr`, `journalctl --user -u macarchy-dfr -n 30`.
...
- [ ] **Step 6 : calibration, à faire avec l'utilisateur.**
  ...
  3. `macarchy-dfr screenshot` après avoir ouvert le groupe Affichage, puis un drag sur le slider de luminosité : `brightnessctl -d apple-panel-bl get` doit avoir changé.
...
**Files:**
- Modify: `~/.config/hypr/bindings.lua` (bloc « Touch Bar (omarchy-dfr) », lignes ~31–47), `~/.config/hypr/autostart.lua` (ligne 5 `o.exec_on_start("omarchy-dfr daemon")`)
- Modify: `~/Work/omarchy-mac/README.md` (pointer vers macarchy-dfr) — la suppression de `hardware/omarchy-dfr` attend le lot 6, quand Jarvis et les autres modules auront migré.
- Create: `README.md` complet du dépôt ; note de mémoire.

- [ ] **Step 1 : retirer le pont F13–F24 et l'ancien autostart.** Dans `bindings.lua`, supprimer le bloc commenté « Touch Bar (omarchy-dfr) » et sa boucle de binds `code:191…202`. Dans `autostart.lua`, remplacer la ligne par `o.exec_on_start("systemctl --user start macarchy-dfr.service")`. `hyprctl reload` puis `hyprctl binds | grep -c 'code:19'` → 0.

- [ ] **Step 2 : arrêter l'ancien daemon.** `kill "$(cat /run/user/$(id -u)/omarchy-dfr/daemon.pid)"` (jamais `pkill -f` depuis un shell dont la ligne contient la chaîne). Vérifier que `macarchy-dfr status` répond et que la barre suit un changement de fenêtre.
...
- [ ] **Step 4 : mémoire.** Mettre à jour `~/.claude/projects/-home-phmatray-Work/memory/omarchy-dfr-touchbar.md` : le daemon est remplacé par macarchy-dfr (dépôt, unité, IPC, `screenshot`/`touch` pour vérifier sans yeux), tiny-dfr masqué ; garder `tiny-dfr-constraints.md` avec une ligne « ne s'applique plus depuis macarchy-dfr, conservé pour mémoire ». Ajouter au `MEMORY.md` la ligne pointant vers la note mise à jour.

- [ ] **Step 5 : pousser.** `gh repo create macarchy/macarchy-dfr --public --source . --push` (l'organisation existe, voir la note `macarchy-org`). Commit dans omarchy-mac : `README: the Touch Bar now lives in macarchy-dfr`.

- [ ] **Step 6 : commit** — `Migrate the session to macarchy-dfr: Hyprland binds, autostart, README`.
